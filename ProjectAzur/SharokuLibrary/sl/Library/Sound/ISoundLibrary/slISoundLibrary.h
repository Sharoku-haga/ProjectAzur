...
public:
	...
	*/
	virtual SoundDataID CreateSoundData(t_char* pFileName) = 0;
	...
	*/
	virtual void PlaybackSound(const SoundDataID& rID, const PLAYBACK_MODE& rMode) = 0;

	/**
	* サウンドを停止する関数
	* @param[in] rID	停止したいサウンドのデータ 
	*/
	virtual void StopSound(const SoundDataID& rID) = 0;

	/**
	* 指定したサウンドデータを停止する関数
	* @param[in] rID	再生したいサウンドのデータ 
	*/
	virtual void ReleaseSoundData(const SoundDataID& rID) = 0;
