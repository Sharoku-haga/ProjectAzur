...
EnergyBlock::EnergyBlock(const sl::DrawingID& rIDs
	...
	, m_pEmptyBlock(new EnergyEmptyBlock(rIDs, rResrcDataFilePath, rGaugePosDiff, oneEnergyBlockTime))
...
bool EnergyBlock::DecreaseEnergy(int elapsedTime)
{
	return m_pEmptyBlock->UpdateWidthSize(elapsedTime);
}

void EnergyBlock::EmptyEnergy()
{
	m_pEmptyBlock->UpdateWidthSizeAll();
}

void EnergyBlock::ChangeColorRed()
{
	std::vector<D3DXCOLOR> color(sl::DX11GraphicsLibrary::Instance().GetModel2DVertexCount()
								, 0xffff0000);
	sl::DX11GraphicsLibrary::Instance().SetDXModel2DColorData(m_pDrawingData->m_IDs.m_ModelID
																, color);
}
